...
%
% Developed by Isaiah Farrell for DAFx24 Surrey, UK
%--------------------------------------------------------------------------
...
%INPUT PARAMETERS                       % NOTE: All linear parameters

INPUT_MODE = 1;                         % INPUT MODE SELECT: 1 = SOUND INPUT, 2 = SINE WAVE TEST TONE
ANALYSIS_MODE = 0;                      % ANALYSIS MODE SELECT: 1 = SPEED (ONLY OUTPUT SIGNAL FFT), 2 = IN-DEPTH (DECOMPOSED SIGNALS AND FILTER RESPONSES)
SOUNDOUT = 1;                           % PLAY SOUND 0 = NO SOUND, 1 = SOUND OUTPUT
SR = 192000;                            % SAMPLE RATE [192K RECOMMENDED FOR EFFECT]
RECORD_OUT = 1;                         % RECORDING MODE SELECT: 0 = NO RECORDING, 1 = RECORD FILES BELOW


% MODE 1
FILENAME = "TESTSIG_LOWGUITAR_48k.wav";       % FILE FOR EFFECT TO BE APPLIED TO
OUTPUT_FILENAME = "TESTS/LOWGUITAR_NEW.wav";  % OUTPUT FILE NAME
...
LOOPTRACK   = 6;        %0-10           % LOOP FILTER CONTROL - INCREASES TRACKING RATE

LOOPSPEED   = 3;        %1-3(INT)       % SPEED OF DECAY FASTEST-SLOWEST - INT VALUE 1,2,3
MULT_PHASE  = 0;        %0/1 (+/-)      % PHASE OF MULTIPLIED SQUARE WAVE 0=POS, 1=NEG
DIV_PHASE   = 1;        %0/1 (+/-)      % PHASE OF DIVIDED SQUARE WAVE 0=POS, 1=NEG

DIVIDER     = 5;        %0-10           % DIVIDED SIGNAL VOLUME
MULTIPLIER  = 2;        %0-10           % MULTIPLIED SIGNAL VOLUME
SQAREWAVE   = 7;        %0-10           % INPUT FREQ SIGNAL VOLUME
...
DIV_INTERVAL = 6;                   % DIVIDED FREQUENCY INTERVAL -
...
tic
%OPEN LOOP SPECS
BIAS = ((TRIGGER-5)/10)*(.6); %+/- .6V                                            % CALCULATE TRIGGER VOLTAGE                          
...
VCO_GAIN_R1 = (10.1-LAGTIME)*50E3;                                         % DEFINE PLL R1  
...
Fgain = (6.2/VCO_GAIN_R1)/(2*(.02E-6)*1.18*.06);                                 % VCO FREQUENCY GAIN COEFFICIENT
phase = 0;
% -----------------------------------------------------------------------

% DECADE COUNTER ---------------------------------------------------------
counter_pulse_mult = floor((5E-5)*SR);                                    % FIXED LENGTH MULTIPLIER COUNTER PULSE 
...
% VOLTAGE CONTROLLED OSCILLATOR ------------------------------------------
    ...
    Fctrl = y_LF*Fgain;                                                     % FREQUENCY GAIN CONTROL

    phaseprev = phase;                                                      % NEW PHASE ACCUMULATION OSCILLATOR METHOD
    phase = mod(phaseprev + 2*pi * (Fctrl/SR),2*pi);
    y_osc = sin(phase);
...
% OUTPUT FILTERING -------------------------------------------------------
    output = y_oscmult*11.185*mult_volume+y_oscdiv*11.185*div_volume...
...
figure
loglog(fvec, mag2db(abs(fft(yt_pre))),'r',fvec, mag2db(abs(fft(yt_mid))),'b','LineWidth',2)
